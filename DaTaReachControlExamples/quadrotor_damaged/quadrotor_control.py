...
from DaTaReachControlExamples.quadrotor_damaged.MyopicDataDrivenControlDaTaControl\
...
res_DaTa_ddc = DaTa_ddc.solve(60,
...
for i, fig in enumerate(save_figs):
    ...
    if not realtime_plot:
        ...
        ax.plot(time_vals, sindyc_traj_vec[:,i], linestyle='-',
            color=sindyc_ddc.marker_color, label=sindyc_ddc.marker_label)
        ax.plot(time_vals, gpyopt_traj_vec[:,i], linestyle='-',
    ...
    if save_plot_tex:
        tikzplotlib.save(log_file+"x_"+str(i)+".tex", figure=fig)
