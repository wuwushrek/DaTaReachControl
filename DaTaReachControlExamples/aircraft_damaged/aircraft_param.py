...
def fFun(x):
    res = np.zeros(len(x))
    res[0] = -0.021 * x[0] + 0.122 * x[1] - 0.322 * x[2]
    res[1] = -0.209 * x[0] - 0.53 * x[1] + 2.21 * x[2]
    res[2] = 0.017 * x[0] + 0.01 * np.cos(x[0]) * x[0] - 0.164 * x[1] +\
             0.15 * np.sin(x[0]) * x[1] - 0.421 * x[2]
    res[3] = x[2]
    res[4] = - x[1] + 2.21 * x[3]
    # res[4] = - x[1]
    return res
...
initial_state = np.array([0,0,0,0,100.0])

# Number of data in initial trajectory
n_data_max = 10

# max number of iteration
max_iteration = 1500 - n_data_max # 200, 500
...
u1_seq = 2 * (np.random.rand(n_data_max,1) - 0.5) * (u1_max/10.0)
u2_seq = (np.random.rand(n_data_max,1) - 0) * (u2_max/10.0)
...
for i in range(1,nSep):
    zero_or_no = np.random.choice([0,1], p=[0.7,0.3])
    if zero_or_no == 0:
        u1_seq[sepIndexes[i],0] = 0
        u2_seq[sepIndexes[i],0] = 0
        continue
    u1_or_u2 = np.random.choice([0,1], p=[0.3,0.7]) # [0.4,0.6]
...
state_lb = np.array([-30.0, -100.0, -20.0,  -40, 0, u1_min, u2_min])
state_ub = np.array([30.0, 100.0, 20.0,  40, 2000, u1_max, u2_max])
print (rand_init_traj_vec[-1,:])

# Planning goals
target_position =np.array([5,5,5,5,200])
target_index = np.array([3]) # The states that matter in the cost function
...
log_file="aircraft_dyn_pitch_only_10_2266"
...
def drawInitialPlot():
    ...
    """
    axes_name = [r'$\mathrm{\omega_l}$', r'$\mathrm{\omega_v}$', r'$\mathrm{q}$',\
                 r'$\mathrm{\theta}$', r'$\mathrm{h}$']
    save_figs = list()
