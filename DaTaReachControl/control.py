...
def computeAffineLinear(x, dt, fOver, GOver, uRange, dtCoeff,
                        ...
                        gronwall=True, term1=True, term2=False):
    ...
    # t1 = np.tensordot(JG_t, np.matmul(GSi, uRange), axes=([1,0]))[:,:,0]
    # t2 = np.matmul(np.tensordot(JG, uRange, axes=([1,0]))[:,:,0], GSi)
    # t3 = np.tensordot(JG_t, fSi, axes=([1,0]))[:,:,0]
    # t4 = np.matmul(Jf, GSi)
    # for i in range(t2.shape[0]):
    #     for j in range(t2.shape[1]):
    #         t2[i,j] = t1[i,j] & t2[i,j]
    # Ai = Gx* dt +(t2 + t3 + t4)* dt_2
    if term1:
        Ai_1 = Gx*dt + (np.tensordot(JG_t, fSi, axes=([1,0]))[:,:,0] + \
                np.matmul(np.tensordot(JG, uRange, axes=([1,0]))[:,:,0]+Jf,
                GSi))* dt_2
    # Another over-approximation of the second order term
    if term2:
        hS = fSi + np.matmul(GSi, uRange)
        Ai_2 = Gx* dt + (np.matmul(Jf, GSi) + np.tensordot(JG_t, hS, axes=([1,0]))[:,:,0])*dt_2
    # print(bi)
    if term1 and term2:
        # print(Ai_1)
        # print(Ai_2)
...
def gurobiControlProblem(bl, Al, uRange, costFun, learningConstr, bothTerm=True):
        ...
        for i, xi in enumerate(x_var):
            ...
            if bothTerm:
                ...
                (mOpt.addLConstr(gp.LinExpr(coeffSup + [-1], u + [xi]),
                        ...
                        name='Up1_{}_{}'.format(counterPb,i)),
                mOpt.addLConstr(gp.LinExpr(coeffInf + [-1], u + [xi]),
                        gp.GRB.LESS_EQUAL, -bl[i,0].lb,
                        name='Lo1_{}_{}'.format(counterPb,i)),
                mOpt.addLConstr(gp.LinExpr(coeffSup + [-1], u + [xi]),
                        gp.GRB.GREATER_EQUAL, -bl[i,0].ub,
                        name='Up2_{}_{}'.format(counterPb,i)),
                mOpt.addLConstr(gp.LinExpr(coeffInf + [-1], u + [xi]),
                        gp.GRB.LESS_EQUAL, -bl[i,0].lb,
                        name='Lo2_{}_{}'.format(counterPb,i)),
                )
            else:
                dictConstr[(counterPb,i)] = \
                (mOpt.addLConstr(gp.LinExpr(coeffSup + [-1], u + [xi]),
        ...
        for j, uj in enumerate(u):
            ...
            if learningConstr is not None:
                dictConstr[(counterPb,-j-1)] = \
                    mOpt.addLConstr(gp.LinExpr([learningConstr[j]], [uj]),
...
def solveGBproblem(bl, Al, learningConstr, dictU, dictConstr,
                    listCost, mOpt, bothTerm=True, verbose=True):
    ...
    for nbProblem, (u, x_var, d) in dictU.items():
        for i , x_i in enumerate(x_var):
            ...
            for j, uj in enumerate(u):
                if d[j]:
                    mOpt.chgCoeff(c1, uj, Al_1[i,j].ub)
                    mOpt.chgCoeff(c2, uj, Al_1[i,j].lb)
                    if Al_2 is not None:
                        mOpt.chgCoeff(c3, uj, Al_2[i,j].ub)
                        mOpt.chgCoeff(c4, uj, Al_2[i,j].lb)
                else:
                    mOpt.chgCoeff(c1, uj, Al_1[i,j].lb)
                    mOpt.chgCoeff(c2, uj, Al_1[i,j].ub)
                    if Al_2 is not None:
                        mOpt.chgCoeff(c3, uj, Al_2[i,j].lb)
                        mOpt.chgCoeff(c4, uj, Al_2[i,j].ub)
        # Constraints on the control variable u
        for j, uj in enumerate(u):
            if learningConstr is not None:
    ...
    #     print(costVal[key], np.array([uVal.x for uVal in val[0] ]))
...
# def solveGBproblem(bl, Al, learningConstr, dictU, dictConstr,
#                     listCost, mOpt, bothTerm=False, verbose=True):
#     """Given the values of Al, bl, the possible imposed learningConstr,
#     compute a solution of the near-optimal control problem
#     """
#     if bothTerm:
...
#     for nbProblem, (u, x_var, d) in dictU.items():
#         for i , x_i in enumerate(x_var):
#             c3 = dictConstr[(nbProblem,i)]
#             c3.RHS = -(bl[i,0].lb+bl[i,0].ub)
#             mOpt.chgCoeff(c3, x_i, -2)
#             for j, uj in enumerate(u):
#                 mOpt.chgCoeff(c3, uj, Al_1[i,j].ub+Al_1[i,j].lb)
#         # Constraints on the control variable u
#         for j, uj in enumerate(u):
#             if learningConstr is not None:
#                 c3 = dictConstr[(nbProblem,-j-1)]
...
#     mOpt.Params.OutputFlag = verbose
#     mOpt.optimize()
#     costVal1 = np.array([obj.getValue() for obj in listCost])
#     ind1 = np.argmin(costVal1)
#     uOpt_1 = np.array([uVal.x for uVal in dictU[ind1][0]])
#     if not bothTerm:
#         return uOpt_1, costVal1

#     for nbProblem, (u, x_var, d) in dictU.items():
#         for i , x_i in enumerate(x_var):
#             c3 = dictConstr[(nbProblem,i)]
#             for j, uj in enumerate(u):
#                 mOpt.chgCoeff(c3, uj, Al_2[i,j].ub+Al_2[i,j].lb)
#     mOpt.Params.OutputFlag = verbose
#     mOpt.optimize()
#     costVal2 = np.array([obj.getValue() for obj in listCost])
#     ind2 = np.argmin(costVal2)
#     uOpt_2 = np.array([uVal.x for uVal in dictU[ind2][0]])
#     if costVal1[ind1] > costVal2[ind2]:
#         return uOpt_1, costVal1[ind1]
#     else:
#         return uOpt_2, costVal2[ind2]
...
#     # for key, val in dictU.items():
#     #     print(costVal[key], np.array([uVal.x for uVal in val[0] ]))
#     ind = np.argmin(costVal1)
#     optU_1 = np.array([uVal.x for uVal in dictU[ind][0]])
#     if not bothTerm:
#         return optU_1, costVal1[ind]
...
#             for j, uj in enumerate(u):
#                 if d[j]:
#                     mOpt.chgCoeff(c1, uj, Al_2[i,j].ub)
#                     mOpt.chgCoeff(c2, uj, Al_2[i,j].lb)
#                 else:
#                     mOpt.chgCoeff(c1, uj, Al_2[i,j].lb)
#                     mOpt.chgCoeff(c2, uj, Al_2[i,j].ub)
#     mOpt.Params.OutputFlag = verbose
#     mOpt.optimize()
#     costVal2 = np.array([obj.getValue() for obj in listCost])
#     # for key, val in dictU.items():
#     #     print(costVal[key], np.array([uVal.x for uVal in val[0] ]))
#     ind2 = np.argmin(costVal2)
#     optU_2 = np.array([uVal.x for uVal in dictU[ind2][0]])
#     print(costVal2[ind2], optU_2)
#     print(costVal1[ind], optU_1)
#     if costVal1[ind] > costVal2[ind2]:
#         return optU_1, costVal1[ind]
#     else:
#         return optU_2, costVal2[ind2]
...
class DaTaControl:
    ...
    def __init__(self, cost_fun, uRange, lipF, lipG, traj={}, nDepf={}, bf={},
                ...
                optVerb=False, solverVerb=False, solopts={}, probLearning=[],
                ...
                widenZeroInterval=1e-3, term1=True, term2=True):
        ...
        # Save which term for the second term to use
        self.term1 = term1
        self.term2 = term2
    ...
    def updateCost(self, costFun):
        ...
        self.dictU, self.dictConstr, self.listCost, self.mOpt = \
            gurobiControlProblem(np.full((self.nState,1),Interval(0)),
                ...
                self.uRange, costFun, np.zeros(self.nControl), self.term1 & self.term2)
    ...
    def __call__(self, currX, currXdot,t=None):
        ...
        bj, Aj = computeAffineLinear(currXInt, self.dt, self.fover,
                ...
                term1=self.term1, term2=self.term2)
        ...
        uOpt, optCost = solveGBproblem(bj, Aj, self.learningConstr, self.dictU,
                                        ...
                                        bothTerm=(self.term1 & self.term2))
        ...
        if self.term1 & self.term2:
            Aj_1, Aj_2 = Aj
            temp1 = bj + np.matmul(Aj_1, self.currentU)
            self.nextStateOverApprox =  bj + np.matmul(Aj_2, self.currentU)
            for i in range(self.nextStateOverApprox.shape[0]):
                self.nextStateOverApprox[i,0] &= temp1[i,0]
        else:
